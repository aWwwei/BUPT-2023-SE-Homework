...
class TempControl:#(QObject):
    ...
    def changeTemp(self):
        t=time.time()
        while True:
            while time.time()-t>=(60 / self.refreshSeq / self.timeSpeed):
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        print('时间',0, '房间',tem[i].roomID, '温度',round(tem[i].tempNow, 4),'目标',tem[i].tempSet, '风速',tem[i].speedSet,'费用',round(tem[i].totalCost, 4),'状态',tem[i].runState)
    strS=''
    strW=''
    ...
    while 1:
        while (time.time()- t)>=10:
            ...
            for i in range(5):
                if msg[i][times] !='':
                    print('——>',i+1,msg[i][times])
            print('时间',times)
            for i in range(5):
                print(tem[i].roomID, '温度',round(tem[i].tempNow, 4),'目标',tem[i].tempSet,'风速',tem[i].speedSet, '费用',round(tem[i].totalCost, 4))
            t3=time.time()
            print(t3-t2)
            strS=''
            strW=''
